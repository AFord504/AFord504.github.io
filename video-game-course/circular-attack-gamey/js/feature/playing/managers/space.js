      ...
      update(event) {
        ...
        for (let i = active.length - 1; i > 0; i--) {
          ...
          if (bodyA.type === "projectile") {
            if (bodyA.emitter.fireType === "homing") {
              homingOn = true;
            } else {
              homingOn = false;
            }
          }
          if (bodyA.type === "projectile" && homingOn) {
            ...
            for (let j = i - 1; j > -1; j--) {
              ...
              if (bodyB.type === "orb") {
                if (
                  getDistanceAttributes(bodyA, bodyB).distance <
                  smallestDistance
                ) {
                  smallestDistance = getDistanceAttributes(
                    bodyA,
                    bodyB
                  ).distance;
                  closestOrb = bodyB;
            ...
            if (bodyA.x < closestOrb.x) {
              bodyA.velocityX += 1;
            } else if (bodyA.x > closestOrb.x) {
              bodyA.velocityX -= 1;
            }
            if (bodyA.y < closestOrb.y) {
              bodyA.velocityY += 1;
            } else if (bodyA.y > closestOrb.y) {
              bodyA.velocityY -= 1;
            }
          }
